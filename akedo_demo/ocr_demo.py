...
if __name__ == '__main__':
    ...
    if PADDLE_AVAILABLE:
        ...
        paddle_msg = '✗ (pip install paddlepaddle paddleocr)'
        if 'PADDLE_ERROR' in globals() and 'langchain' in PADDLE_ERROR:
            paddle_msg = '✗ (dependency issue: pip install langchain-community)'
        print(f"  PaddleOCR:  {paddle_msg}")
    print(f"  Florence-2: {'✓ (SOTA Vision-Language Model)' if FLORENCE_AVAILABLE else '✗ (pip install transformers torch)'}")
    print()
    ...
    if TESSERACT_AVAILABLE:
        ...
        for item in extract_items_from_text(text)[:10]:
            if item.get('price'):
                print(f"  {item['quantity']:>2}x {item['name']:<40} £{item['price']:.2f}")
            else:
    ...
    if FLORENCE_AVAILABLE:
        print("-" * 60)
        print("Method 4: FLORENCE-2 (SOTA Vision-Language Model, ~770MB)")
        print("-" * 60)
        print("Florence-2 is Microsoft's state-of-the-art vision model.")
        print("It understands document structure, not just text pixels.")
        print("Expected: 30-40% better accuracy than traditional OCR\n")

        import time
        start_time = time.time()
        text = florence_ocr(IMAGE_PATH, model_size='large', use_gpu=True)
        elapsed = time.time() - start_time

        print("Raw OCR Output:")
        print(text[:800] if len(text) > 800 else text)
        print("\nExtracted Items:")
        florence_items = extract_items_from_text(text)
        for item in florence_items[:10]:
            if item.get('price'):
                print(f"  {item['quantity']:>2}x {item['name']:<40} £{item['price']:.2f}")
            else:
                print(f"  {item['quantity']:>2}x {item['name']}")
        print(f"\n⚡ Processing time: {elapsed:.2f}s")
        print(f"Total items found: {len(florence_items)}")
        print()

    # PRODUCTION METHOD - Optimized for speed!
    print("=" * 60)
    print("⭐ PRODUCTION PIPELINE - FAST MODE (RECOMMENDED) ⭐")
    print("=" * 60)
    print("Production-grade preprocessing (OPTIMIZED FOR SPEED):")
    print("  ✓ EXIF rotation (automatic)")
    print("  ✓ Illumination normalization")
    print("  ✓ Adaptive thresholding")
    print("  ✓ Morphological cleanup")
    print("  ✓ Auto-cropping")
    print("  ⏭  Deskewing (DISABLED for speed, enable if needed)")
    print("  ⏭  Perspective correction (DISABLED for speed, enable if needed)")
    print()

    if TESSERACT_AVAILABLE or EASYOCR_AVAILABLE or PADDLE_AVAILABLE:
        import time
        start_time = time.time()
        # FAST MODE: perspective and deskewing disabled by default
        text = production_ocr(IMAGE_PATH, engine='auto', 
        ...
        for item in items[:15]:
            if item.get('price'):
                ...
                print(f"  {item['quantity']:>2}x {item['name']}")
        print(f"\n⚡ Processing time: {elapsed:.2f}s (FAST MODE)")
        print(f"Total items found: {len(items)}")
        print()
    else:
        print("No OCR engine available for production pipeline demo")
        print()
    ...
    result_standard = process_receipt_edge(IMAGE_PATH, method='auto', use_production=False)
    if 'error' not in result_standard:
        print(f"Method: {result_standard['method_used']}")
        print(f"Preprocessing: {result_standard['preprocessing']}")
        print(f"Items found: {result_standard['item_count']}")
        print("\nItems:")
        for item in result_standard['items'][:10]:
            if item.get('price'):
                print(f"  {item['quantity']:>2}x {item['name']:<40} £{item['price']:.2f}")
            else:
                print(f"  {item['quantity']:>2}x {item['name']}")
    ...
    print("\n--- Production Preprocessing FAST MODE (RECOMMENDED) ---")
    import time
    start = time.time()
    result_production = process_receipt_edge(IMAGE_PATH, method='production',
                                            enable_perspective=False,
                                            enable_deskewing=False,
                                            enable_sharpening=False)
    fast_time = time.time() - start
    
    if 'error' not in result_production:
        print(f"Method: {result_production['method_used']}")
        print(f"Preprocessing: {result_production['preprocessing']} (FAST)")
        print(f"Items found: {result_production['item_count']}")
        print(f"⚡ Time: {fast_time:.2f}s")
        print("\nItems:")
        total_price = 0.0
        for item in result_production['items'][:10]:
            if item.get('price'):
                print(f"  {item['quantity']:>2}x {item['name']:<40} £{item['price']:.2f}")
        ...
        if total_price > 0:
            print(f"\n  {'Total (displayed items):':<45} £{total_price:.2f}")

        # Show improvement
        if 'error' not in result_standard:
            improvement = result_production['item_count'] - result_standard['item_count']
            if improvement > 0:
                print(f"\n✅ Production preprocessing found {improvement} more items!")
            elif improvement < 0:
                print(f"\n⚠️  Standard found {abs(improvement)} more items")
            else:
                print(f"\n➡️  Same number of items found")
    else:
        print(f"Error: {result_production.get('error', 'Unknown error')}")

    # Florence-2 comparison (if available)
    if FLORENCE_AVAILABLE:
        print("\n--- Florence-2 Vision-Language Model (BEST ACCURACY) ---")
        start = time.time()
        result_florence = process_receipt_edge(IMAGE_PATH, method='florence', florence_model_size='large')
        florence_time = time.time() - start

        if 'error' not in result_florence:
            print(f"Method: {result_florence['method_used']}")
            print(f"Preprocessing: {result_florence['preprocessing']}")
            print(f"Items found: {result_florence['item_count']}")
            print(f"⚡ Time: {florence_time:.2f}s")
            print("\nItems:")
            total_price = 0.0
            for item in result_florence['items'][:10]:
                if item.get('price'):
                    print(f"  {item['quantity']:>2}x {item['name']:<40} £{item['price']:.2f}")
                    total_price += item['price'] * item['quantity']
                else:
            ...
            if 'error' not in result_standard:
                improvement = result_florence['item_count'] - result_standard['item_count']
                if improvement > 0:
                    print(f"\n🚀 Florence-2 found {improvement} more items than standard OCR!")
                elif improvement < 0:
                    print(f"\n⚠️  Standard found {abs(improvement)} more items")
                else:
                    print(f"\n➡️  Same number of items as standard")
        else:
            print(f"Error: {result_florence.get('error', 'Unknown error')}")