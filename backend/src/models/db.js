...
class Inventory {
  ...
  static async findByName(userId, itemName) {
    try {
      // For encrypted data, fetch all user items and filter in memory
      const allItems = await this.findByUser(userId);
      return allItems.find(item => item.item_name === itemName);
    } catch (error) {
...
class Orders {
  ...
  static async create(orderData) {
    ...
    try {
      ...
      const result = await db.query(
        `INSERT INTO orders
         (user_id, vendor, items, subtotal, tax, shipping, total, status, is_encrypted)
